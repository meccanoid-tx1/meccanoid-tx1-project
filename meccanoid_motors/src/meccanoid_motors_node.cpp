#include <ros/ros.h>
#include "meccanoid_motor_control.h"
#include <meccanoid_motors/meccanoid_motor_control.h>

/*
/ Motor class 
*/
class MeccanoidMotors{
  public:
    /*
    / Constructor
    */
    MeccanoidMotors(){
      controller = createMeccanoidMotorControl();
      ROS_INFO("motors dummy: num motors %d", controller->numMotors);
    }

    /*
    / Motor control callback
    */
    void motorControlCallback(const meccanoid_motors::meccanoid_motor_controlConstPtr& msg){
        if(controller->active){
            //TODO: implement motor control logic
        }
        ROS_INFO("motor_control callback");
    }
  
  private:
    MeccanoidMotorControl* controller = NULL;
    meccanoid_motors::meccanoid_motor_control motor_control_msg;
};

/*
/ Meccanoid motor control node main 
*/
int main(int argc, char **argv){
    // init ros
    ros::init(argc, argv, "meccanoid_motors");
    ros::NodeHandle nodeHandle;

    // set up face localization subscriber
    MeccanoidMotors *motors = new MeccanoidMotors();
    ros::Subscriber motorControlSubscriber = nodeHandle.subscribe("/motor_control", 1, 
                                                  &MeccanoidMotors::motorControlCallback, motors);
