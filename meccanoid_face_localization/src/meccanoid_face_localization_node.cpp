#include <ros/ros.h>
#include "meccanoid_servo_control.h"
#include <meccanoid_face_localization/meccanoid_face_location.h>


/*
/ Face localization class 
*/
class MeccanoidFaceLocalization{
  public:
    /*
    / Constructor
    */
    MeccanoidFaceLocalization(){
      controller = createMeccanoidServoControl();
      ROS_INFO("face localization dummy: num servos %d", controller->numServos);
    }

    /*
    / Face localization callback
    */
    void faceLocalizationCallback(const meccanoid_face_localization::meccanoid_face_locationConstPtr& msg){
        if(controller->active){
          //TODO: implement servo localization logic
        }
        ROS_INFO("face_localization callback");
    }
  
  private:
    MeccanoidServoControl* controller = NULL;
    meccanoid_face_localization::meccanoid_face_location face_location_msg;
};
...
int main(int argc, char **argv){
    ...
    // set up face localization subscriber
    MeccanoidFaceLocalization *faceLocalizer = new MeccanoidFaceLocalization();
    ros::Subscriber faceLocalizationSubscriber = nodeHandle.subscribe("/face_location", 1, 
                                                  &MeccanoidFaceLocalization::faceLocalizationCallback, faceLocalizer);
...
/*######################################################################################
/ ###################  TODO: ADAPT MECCABRAIN METHODS TO WORK ##########################
/ ######################################################################################
/ ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
