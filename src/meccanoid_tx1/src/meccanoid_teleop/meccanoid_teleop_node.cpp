...
class MeccanoidTeleop {
	...
	public:
	MeccanoidTeleop(ros::Publisher pub, ros::Publisher pub2){
		keyPublisher = pub;
		modePublisher = pub2;
		mode = false;
	};
    ...
    bool teleoperate(){
        ...
        switch(getkey()){
            ...
            case 27: // ctrl+c
                ...
                break;
            default:
                ROS_INFO("\nldir: %d, rdir: %d,\n lbreak: %d, rbreak: %d,\n lspeed: %d, rspeed: %d, mode: %s",_l_dir, _r_dir, _l_break, _r_break, _l_speed, _r_speed, (mode)?"auto":"manual");
        }
        return operate;
    }
	...
	void publishMode(){
		...
		auto_mode_msg.data = mode;
		modePublisher.publish(auto_mode_msg);
	}
	...
	void forward_left(){
        ...
        keyPublisher.publish(message(1,1,0,0,125,255));
    }
	...
	void forward_right(){
        ...
        keyPublisher.publish(message(1,1,0,0,225,125));
    }
...
int main(int argc, char **argv){
	...
	ros::Publisher modePublisher = nodeHandle.advertise<std_msgs::Bool>("operation_mode",1);
    ros::Rate loop_rate(10);

    MeccanoidTeleop *teleop = new MeccanoidTeleop(keyPublisher,modePublisher);
